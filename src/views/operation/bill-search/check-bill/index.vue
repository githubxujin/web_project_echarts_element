<template>
  <div class="check-bill u-layout-container">
...
import { getCheckBillList, getDeviceTree, revokeCheckBill, checkDispatch, repairTransfer, checkAudit, getCheckAuditMsg, repairSuspend } from '../../../../services/operation';
import { CheckBillStatus, isOverLimit, CheckBillStatusEnum } from '../../../../enum/operation-enum';
...
export default {
  ...
  data () {
    return {
      ...
      status: [-1], //状态
      statusOptions: CheckBillStatus,
      BillStatusEnum: CheckBillStatusEnum,//工单状态枚举
      tableData: [],
      curBill: {},//当前选中工单
      treeData: [],
      defaultProps: {
  ...
  methods: {
    ...
    //打开撤单审核弹窗
    openCancleAudit (row) {
      getCheckAuditMsg(row.billNumber).then(res => {
        //console.log('res', res);
        if (res.code == 200) {
