  ...
  <div class="water-editor">
    ...
    >
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item prop="drainageDeviceId" label="所属设备">
            <TreeSelect
              class="project-tree"
      ...
      <el-row
        ...
        v-for="(item, index) in form.drainagePointTableMappingList"
        :key="item.copyId || item.id"
      >
        <el-col :span="12">
          <el-form-item
            class="is-required"
            :prop="`drainagePointTableMappingList[${index}].returnValue`"
            label="返回值"
            :rules="[{
        ...
        <el-col :span="12">
          <el-form-item
            class="is-required"
            :prop="`drainagePointTableMappingList[${index}].returnValueDesc`"
            label="值含义"
            :rules="[{
                      validator: (rule, value, callback) => {
...
import { drainagePointTableGetInfo, drainagePointTableAdd, drainagePointTableEdit, configTypeQuery, tachometerGetArray } from '@/services/system-settings.js'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      form: {
        ...
        upperLimit: '',
        lowerLimit: '',
        drainagePointTableMappingList: [JSON.parse(JSON.stringify(this.paramObj))],
        unit: '',
        status: 0,
        remark: '',
        shopNumber: ''
      ...
      formRules: { // 表单规则
        ...
        drainageDeviceId: [{ required: true, message: '请选择所属设备', trigger: 'change' }],
  ...
  computed: {
    ...
    isEdit () { // 判断是否编辑
      return Boolean(this.waterId || this.waterId === 0)
    },
    params () {
    ...
    mappingValueList () {
      const MappingValueList = this.form.drainagePointTableMappingList.filter(item => {
    ...
    mappingDescList () {
      const MappingDescList = this.form.drainagePointTableMappingList.filter(item => {
  ...
  methods: {
    drainagePointTableGetInfo, //获取给排水详情
    drainagePointTableAdd, // 新增给排水参数
    drainagePointTableEdit, // 编辑给排水参数
    configTypeQuery,
    tachometerGetArray,
    initData () {
      ...
      axios.all([this.configTypeQuery({ configType }), this.tachometerGetArray({ shopNumber, deviceType: -1 })]).then(axios.spread((...args) => {
        ...
        this.dialogLoading = false
        console.error('获取单位列表或给排水设备列表或采集命令列表失败(errorMessage)：', _);
      })
    },
    addFormParam () {
      this.form.drainagePointTableMappingList.push(JSON.parse(JSON.stringify(this.paramObj)))
    },
    deleteFormParam (index) {
      this.form.drainagePointTableMappingList.splice(index, 1)
    },
    submit () { // 提交编辑或新增
      let result = false
      ...
      if (!result) return
      let funName = 'drainagePointTableEdit'
      if (!this.isEdit) {
      ...
      this[funName](this.params).then(res => {
        this.dialogLoading = false
        this.$message.success(this.isEdit ? '编辑成功！' : '新增给排水参数成功！')
        this.$emit('success')
        this.cancle()
      }).catch(_ => {
        console.error('编辑给排水参数失败(errorMessage):', _);
        this.dialogLoading = false
      })
    },
    cancle () { // 重置表单并关闭表单
      ...
      this.form.drainagePointTableMappingList = [JSON.parse(JSON.stringify(this.paramObj))]
      this.$nextTick(_ => {
        this.$refs.form && this.$refs.form.clearValidate()
      })
      this.$emit('update:waterId', null)
  ...
  watch: {
    editDialogVisible: { // 监听editDialogVisible，获取给排水参数信息
      immediate: true,
      deep: true,
      handler: function (visible) {
        if (!visible || !this.waterId && this.waterId !== 0) return
        this.drainagePointTableGetInfo({ id: this.waterId }).then(res => {
          let data = res.data || {}
          Object.keys(this.form).forEach(prop => {
            let value = data[prop]
            this.form[prop] = value
          }, this)
          if (!this.form.drainagePointTableMappingList || !this.form.drainagePointTableMappingList.length) {
            this.form.drainagePointTableMappingList = [JSON.parse(JSON.stringify(this.paramObj))]
          }
          this.$nextTick(_ => {
            this.$refs.form && this.$refs.form.clearValidate()
