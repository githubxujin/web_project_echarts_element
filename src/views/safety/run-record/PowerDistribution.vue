          ...
          <div class="title-input-group u-title-input-group">
            ...
            <div class="input-container">
              <div style="border-radius: 2px;"
                ...
                <el-select v-model="meterId"
                           @change="getNumberParamOptions">
                  <el-option v-for="item in meterOptions"
                             :key="item.meterId"
                             :label="item.name"
                             :value="item.meterId"></el-option>
                </el-select>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    init () {
      this.paramId = null; this.meterId = null; this.numberParam = [];
          ...
          getMeterOfPowerSupple({ shopNumber: this.shopNumber, transformerId: this.paramId }).then((res) => {
            if (res.data.length > 0) {
              ...
              getParamOfPowerSupple(this.meterId).then((res) => {
                if (res.data.length > 0) {
                  this.numberParam.push(res.data[0].paramId);
      ...
      this.meterId = null; this.numberParam = [];
      getMeterOfPowerSupple({ shopNumber: this.shopNumber, transformerId: this.paramId }).then((res) => {
        if (res.data.length > 0) {
          this.meterId = res.data[0].meterId;
          this.meterOptions = res.data;
    ...
    query () {
      ...
      let t_numberParam = this.numberParam;
      ...
      if (!this.isdata) {
        ...
        getPowerSupplyDistributionGraph(o).then((res) => {
          ...
          for (let i = 0; i < this.numberParam.length; i++) {
            ...
            for (j; j < this.numberParamOptions.length; j++) {
              if (iv == this.numberParamOptions[j].paramId) {

                if (res.data.runningData.length == 0) { continue; }

                for(let l=0;l<res.data.runningData.length;l++){
                  if(res.data.runningData[l].hasOwnProperty(this.numberParamOptions[j].name)) {
                        let rd =res.data.runningData[l][this.numberParamOptions[j].name];
                    ...
                    // continue;
                        ...
                        this.chartOption.legend.data.push(this.numberParamOptions[j].name);
                        // console.log(JSON.stringify(rd))
                        let param = {
                            name: this.numberParamOptions[j].name,
                            type: 'line',
                            smooth: true,
                            color: colors[i],
                            data: []
                ...
                // let tname=this.numberParamOptions[j].name;
                // let rd = res.data.runningData[this.numberParamOptions[j].name];
                //   if(rd.length==0){ continue; }
                // this.chartOption.legend.data.push(this.numberParamOptions[j].name);
                // // console.log(JSON.stringify(rd))
                // let param = {
          ...
          for (let i = 0; i < this.numberParam.length; i++) {
            ...
            for (j; j < this.numberParamOptions.length; j++) {
              if (iv == this.numberParamOptions[j].paramId) {
                ...
                this.tHead.push({ text: this.numberParamOptions[j].name, prop: this.numberParamOptions[j].name })
