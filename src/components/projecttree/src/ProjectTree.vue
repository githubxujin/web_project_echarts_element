...
/**
     ...
     * 支持是否复选isMultiple
     * 支持是否默认全选
     * 项目树数据请求失败会触发ajaxError事件,并返回错误信息
     */
export default {
  name: 'ElFormItem',
  ...
  props: {
    ...
    dataUrl: { // dataArray为空情况下请求项目树数据请求URL
      type: String,
      default: '/system-setting/shop/getShopTree'
    },
    ajaxFun: {
      type: String,
      default: 'post'
    },
    requestParams: {  // 项目树数据请求参数  // treeType{1:不控权限,2:控权限}
      type: Object,
      default: () => {
        return {
          treeType: 2
  ...
  computed: {
    ...
    selectedLabel () {
      ...
      checkedItem = this.checkStrictly ? checkedItem : checkedItem.filter(item => !item.children)
      ...
      switch (checkedLen) {
        ...
        default:
          return this.checkedLabel || this.someCheckedLabel || `已选择门店${checkedLen}个`;
      }
    },
    selectSize () {
      // 项目数选择器大小
    ...
    refreshShopTreeFlag () {
      return this.$store.getters.getRefreshShopTreeFlag;
    }
  },
  watch: {
    visible (val) {
    ...
    value: {
      ...
      handler: function (val = []) {
        ...
        if (this.anotherStyle && val.length == 0) { //长度为0时提示选择，仅限于首页的
          this.$message({
            message: '取消全选时默认展示所有门店的数据',
            type: 'warning',
    ...
    //刷新门店树
    refreshShopTreeFlag (val) {
      console.log('refreshShopTreeFlag：刷新 :', val);
      if (val) {
        this.initData();
      }
    }
  ...
  methods: {
    initData (url = this.dataUrl) { // 初始化项目树列表数据
    ...
    treeCheckChange (...args) { // 处理多选变化
      // console.log('都选变化', args)
      // 项目树选中状况变化回调
      if (this.isMultiple) {
        // 复选状态下，只要项目树选中状态有变化就返回当前选中值
        const checkNodes =
          this.$refs.elTree && this.$refs.elTree.getCheckedNodes();
        // console.log('没有处理的数据', checkNodes)
        const checkNodesCopy = this.checkStrictly ? checkNodes : checkNodes.filter(item => !item.children);
        // console.log('处理过的数据', checkNodesCopy)
    ...
    renderContent (h, { node, data, store }) {
      let _color = this.anotherStyle ? 'color:white;' : 'color:black;';
      return (
        < span
          class
          ="el-tree-node__label"
...
$background: #14223c;
$borderColor: #67cbff;
$border2: 1px solid rgba(103, 203, 255, 0.5); //1px solid $borderColor;
