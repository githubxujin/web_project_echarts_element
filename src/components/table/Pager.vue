...
export default {
  ...
  computed: {
    ...
    // 检测获取到的数据是否为空
    isEmptyList () {
      return Math.ceil(this.pager[this.props.total] / this.pager[this.props.pageSize]) < this.pager[this.props.pageNum];
    },
  },
  watch: {
    total () {
  ...
  methods: {
    ...
    onChangeSize (pageSize) {
      const temp = {
        ...
        // 当显示条数小于或等于总条数时，重置页数
        [this.props.pageNum]: pageSize <= this.total ? 1 : this.pager[this.props.pageNum],
      };
    ...
    onChangePage (pageNum) {
      this.$emit('setPager', Object.assign(this.pager, { [this.props.pageNum]: pageNum }));
      this.$emit('query');
    },
  },
};
