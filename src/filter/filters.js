...
//不需要权限验证的路由地址
const noAuth = ['/settings/basics/new-alarm', '/digital-signage', '/auto-login']
const urlArray = ['/shop-index', '/safety/alarm/view']
// 使用钩子函数对路由进行权限跳转
router.beforeEach((to, from, next) => {
    // console.log('前');
    // if (to == '/auto-login' && urlArray.includes(from)) {
    ...
    if (!userInfo) {
        if (to.path !== '/login' && !noAuth.includes(to.path)) {
            next('/login')
            return
        }
        next()
